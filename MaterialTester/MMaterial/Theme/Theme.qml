...
MMaterial.ThemeBase{
    ...
    property MMaterial.ThemeBase currentTheme: DarkTheme
    property var chartColors: [root.primary, root.secondary, root.info, root.success, root.warning, root.error]

    function setTheme(theme: MMaterial.ThemeBase) : void {
        console.log("Theme switched to " + theme.objectName)
        currentTheme = theme;
    }

    function getChartColorPattern(index: int) : MMaterial.PaletteBasic {
        return chartColors[index % chartColors.length];
    }

    function getChartPatternColor(index: int, pattern: MMaterial.PaletteBasic) : color {
